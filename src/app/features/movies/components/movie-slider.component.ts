...
import { SkeletonSliderComponent } from '../../../shared/components/skeleton/skeleton-slider.component';
...
@Component({
	...
	standalone: true,
	imports: [CommonModule, RouterLink, SkeletonSliderComponent],
	...
	template: `
		<div class="relative w-full my-6">
			<!-- Skeleton loading state -->
			@if (isLoading()) {
				<app-skeleton-slider [itemCount]="skeletonItemCount()" />
			} @else {
				<!-- Left Arrow -->
				@if (!isAtStart()) {
					<button 
...
export class MovieSliderComponent {
	...
	@Input({ required: false }) skeletonItemCountInput?: number;
	...
	skeletonItemCount(): number {
		return this.skeletonItemCountInput || 6;
	}
